...
<div class="gp-doc-container gp-content">
    ...
    <code><pre>$db = GpDBMySql::getInstance();
$db = Gp::db();
$db = new GpDBMySql($ip, $name, $psw, $dbName);</pre></code>
    ...
    <h3>connect: Collegarsi al database</h3>
    <code><pre>$db->connect($ip, $login, $pass, $dbname);</pre></code>
    <p>Si collega al database. Ritorna Boolean e nel caso di fallimento setta la variabile $db->error = true;
