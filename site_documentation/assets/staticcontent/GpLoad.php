...
Gp::load();</pre></code>
    ...
    <code><pre>Gp::load()->setPath($varName, $path )</pre></code>
    <p>
    ...
    Nell'esempio MyPage imposta un percorso in cui verranno cercati i file prima dentro la directory 'override/pages' e se non si trovano dentro
        <code><pre>Gp::load()->setPath('myPage', array('override/pages', 'pages')) ;</pre></code>
    </p>
    ...
    <code><pre>Gp::load()->issetPath($varName)</pre></code>

    <h3><b>getUri</b>: Ritorna il link relativo alla pagina settata</h3>
    <code><pre>Gp::load()->getUri($varName, $filename = "");</pre></code>
    <p>
    ...
    Ritorna il link del file cercato o il link della directory caricabile dal sito www.miosito.xx/pages
        <code><pre>Gp::load()->setPath('myPage', array('pages', 'themes/easy/pages'));
echo Gp::load()->getUri('myPage');</pre></code>
    </p>
   
    <h3><b>append</b>: Aggiunge un segmento ad un path esistente</h3>
    <code><pre>Gp::load()->append($newName, $varName, $appendPath);</pre></code>

    <h3><b>getPath</b>: Ritorna la stringa con il percorso del file </h3>
   
    <code><pre>Gp::load()->getPath($varName="", $fileName= ""); 
    </pre></code>
    ...
    <code><pre>Gp::load()->require($varName, $fileName = "", $data = false, $requireOnce = false, $variable = false);
        </pre></code>
  ...
  <p>
	 ...
	 <b>$variable </b> String Il nome della variabile in cui settare i dati. di default i dati sono settati in $this->cData<br>
      ...
      <h3><b>getFile </b>: Carica un file utilizzando il sistema di percorsi e override di default</h3>
    <code><pre>Gp::load()->getFile($varName, $fileName);
        </pre></code>
        <p>Carica un file usando file_get_contents. Se viene passato solo un parametro questo deve essere il path assoluto</p>
  <p>
     <b>$varName</b>String  il nome del gruppo di directory da richiamare oppure se fileName = "" allora il percorso completo del file da richiamare<br>
	 <b>$fileName</b> String|Array  Opzionale Il nome del file o dei files secondo l'ordine di getPath<br>
     </p>
 ...
 <code><pre>Gp::load()->module($moduleName, $returnType = "html", $data = false);
    </pre></code>
